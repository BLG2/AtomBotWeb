...
namespace AtomWeb.Controllers
{
    ...
    public class ServerStatsController : Controller
    {
        ...
        public ServerStatsController(MongoDbApiServices mongoDbServices, DiscordBotApiServices discordBotApiServices)
        {
        ...
        public async Task<IActionResult> Index(string guildId)
        {
            ...
            {
                DbName = "AtomBot",
                CollectionName = "ServerStats",
                SearchKey = new { GuildId = guildId }
            });
            var model = JsonConvert.DeserializeObject<ServerStatsVM>(antiApiRes);
            if (model == null) model = new ServerStatsVM { GuildId = guildId };
            model.GuildInfo = guildInfo;

            ViewData["Notify"] = PopupMessageService.GetPupupMessage(this);
            ViewData["BreadCrumb"] = BreadCrumbsService.AddBreadCrumbAsync(this, "ServerStats Settings");
        ...
        public async Task<IActionResult> SaveForm(ServerStatsVM vm)
        {
            var accesToken = CookieService.GetSignedInAccesToken(HttpContext);
            if (accesToken == null) throw new Exception("Cound not fetch Auth2 AccesToken pls re-signin");
            var discordUser = await DiscordAuth.GetUserWithAccesTokenAsync(accesToken?.access_token ?? "123");
            ...
            if (ModelState.IsValid)
            {
                var antiApiRes = await _mongoDbServices.UpdateDbObjectAsync(new ApiPostModel
                {
                    DbName = "AtomBot",
                    CollectionName = "ServerStats",
                    ModelObject = JsonConvert.DeserializeObject<ServerStats>(JsonConvert.SerializeObject(vm)),
                });
