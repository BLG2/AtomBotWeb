...
namespace AtomWeb.Controllers
{
    ...
    public class SelfRoleController : Controller
    {
        private readonly MongoDbApiServices _mongoDbServices;
        private readonly DiscordBotApiServices _discordBotApiServices;

        public SelfRoleController(MongoDbApiServices mongoDbServices, DiscordBotApiServices discordBotApiServices)
        {
        ...
        public async Task<IActionResult> Index(string guildId)
        {
            ...
            {
                DbName = "AtomBot",
                CollectionName = "SelfRole",
                SearchKey = new { GuildId = guildId }
            });
            var model = JsonConvert.DeserializeObject<SelfRoleVM>(antiApiRes);
            if (model == null) model = new SelfRoleVM { GuildId = guildId };
            model.GuildInfo = guildInfo;

            ViewData["Notify"] = PopupMessageService.GetPupupMessage(this);
            ViewData["BreadCrumb"] = BreadCrumbsService.AddBreadCrumbAsync(this, "Selfrole Settings");
        ...
        public async Task<IActionResult> SaveForm(SelfRoleVM vm)
        {
            var accesToken = CookieService.GetSignedInAccesToken(HttpContext);
            if (accesToken == null) throw new Exception("Cound not fetch Auth2 AccesToken pls re-signin");
            var discordUser = await DiscordAuth.GetUserWithAccesTokenAsync(accesToken?.access_token ?? "123");
            ...
            if (ModelState.IsValid)
            {
                var antiApiRes = await _mongoDbServices.UpdateDbObjectAsync(new ApiPostModel
                {
                    DbName = "AtomBot",
                    CollectionName = "SelfRole",
                    ModelObject = JsonConvert.DeserializeObject<SelfRole>(JsonConvert.SerializeObject(vm)),
                });
        ...
        {
            await _discordBotApiServices.SendDiscordMessageAsync(guildId, AtomData.Enums.DiscordMessageApiTypeEnum.SelfRole);
