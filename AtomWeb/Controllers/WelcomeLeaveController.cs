...
namespace AtomWeb.Controllers
{
    ...
    public class WelcomeLeaveController : Controller
    {
        private readonly MongoDbApiServices _mongoDbServices;
        private readonly DiscordBotApiServices _discordBotApiServices;

        public WelcomeLeaveController(MongoDbApiServices mongoDbServices, DiscordBotApiServices discordBotApiServices)
        {
        ...
        public async Task<IActionResult> Index(string guildId)
        {
            ...
            {
                DbName = "AtomBot",
                CollectionName = "WelcomeLeave",
                SearchKey = new { GuildId = guildId }
            });
            var model = JsonConvert.DeserializeObject<WelcomeLeaveVM>(antiApiRes);
            if (model == null) model = new WelcomeLeaveVM { GuildId = guildId };
            model.GuildInfo = guildInfo;

            ViewData["Notify"] = PopupMessageService.GetPupupMessage(this);
            ViewData["BreadCrumb"] = BreadCrumbsService.AddBreadCrumbAsync(this, "Welcome-Leave Settings");
        ...
        public async Task<IActionResult> SaveForm(WelcomeLeaveVM vm)
        {
            var accesToken = CookieService.GetSignedInAccesToken(HttpContext);
            if (accesToken == null) throw new Exception("Cound not fetch Auth2 AccesToken pls re-signin");
            var discordUser = await DiscordAuth.GetUserWithAccesTokenAsync(accesToken?.access_token ?? "123");
            ...
            if (ModelState.IsValid)
            {
                var antiApiRes = await _mongoDbServices.UpdateDbObjectAsync(new ApiPostModel
                {
                    DbName = "AtomBot",
                    CollectionName = "WelcomeLeave",
                    ModelObject = JsonConvert.DeserializeObject<WelcomeLeave>(JsonConvert.SerializeObject(vm)),
                });
        ...
        public async Task<IActionResult> SetWelcomeMessage(string guildId)
        {
            var accesToken = CookieService.GetSignedInAccesToken(HttpContext);
            if (accesToken == null) throw new Exception("Cound not fetch Auth2 AccesToken pls re-signin");
            var discordUser = await DiscordAuth.GetUserWithAccesTokenAsync(accesToken?.access_token ?? "123");
            ...
            ViewData["Notify"] = PopupMessageService.GetPupupMessage(this);
            ViewData["BreadCrumb"] = BreadCrumbsService.AddBreadCrumbAsync(this, "Welcome Message");

            return View(model);
        }

        public async Task<IActionResult> SetLeaveMessage(string guildId)
        {
            var accesToken = CookieService.GetSignedInAccesToken(HttpContext);
            if (accesToken == null) throw new Exception("Cound not fetch Auth2 AccesToken pls re-signin");
            var discordUser = await DiscordAuth.GetUserWithAccesTokenAsync(accesToken?.access_token ?? "123");
            ...
            ViewData["Notify"] = PopupMessageService.GetPupupMessage(this);
            ViewData["BreadCrumb"] = BreadCrumbsService.AddBreadCrumbAsync(this, "Leave Message");
